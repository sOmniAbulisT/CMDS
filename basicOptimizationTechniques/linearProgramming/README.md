...
Linear programming is a foundational mathematical method used to determine the best possible outcome (such as maximum profit or lowest cost)
in a given mathematical model where the requirements are represented by linear relationships. It is widely used in resource allocation, 
operations research, and as a building block for more complex statistical and machine learning algorithms. 
...
3. The script will automatically compile `src/optimize.cpp` and output the step-by-step matrix transformations and the final 
optimal solution.
